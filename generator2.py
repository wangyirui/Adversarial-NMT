...
class RNNModel(nn.Module):
    ...
    def forward(self, sample):
        ...
        if self.args.bidirectional:
            x, final_hiddens = encoder_out
            encoder_hiddens = torch.cat([final_hiddens[0:final_hiddens.size(0):2], final_hiddens[1:final_hiddens.size(0):2]], 2)
            encoder_out = (x, encoder_hiddens)
...
class GRUEncoder(nn.Module):
    """GRU encoder."""
    def __init__(self, dictionary, embed_dim=512, num_layers=1, dropout_in=0.1, dropout_out=0.1, bidirectional=False):
        ...
        self.gru = GRU(
            ...
            hidden_size=embed_dim // 2 if bidirectional else embed_dim,
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        h0 = Variable(x.data.new(self.num_layers * direction, bsz, embed_dim // direction).zero_())
        x, (final_hiddens) = self.gru(x,h0)
...
class GRUDecoder(nn.Module):
    ...
    def forward(self, prev_output_tokens, encoder_out, incremental_state=None):
        ...
        if cached_state is not None:
            prev_hiddens = list(cached_state)
        else:
        ...
        utils.set_incremental_state(
            self, incremental_state, 'cached_state', prev_hiddens)
